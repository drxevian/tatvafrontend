...
import { ServiceInquiry, getAllServiceInquiries, updateServiceInquiryStatus, deleteServiceInquiry } from "@/utils/serviceInquiries";
...
const ServiceInquiriesPage = () => {
  ...
  useEffect(() => {
    const fetchInquiries = async () => {
      try {
        const data = await getAllServiceInquiries();
        setInquiries(data);
      } catch (error) {
  ...
  const filteredInquiries = inquiries.filter(
    (inquiry) =>
      ...
      inquiry.serviceDetails.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleViewInquiry = async (inquiry: ServiceInquiry) => {
    setSelectedInquiry(inquiry);
    
    // Mark as read if it's new
    if (inquiry.status === "new") {
      try {
        const updatedInquiry = await updateServiceInquiryStatus(inquiry.id, "read");
  ...
  const handleDeleteConfirm = async () => {
    if (inquiryToDelete) {
      try {
        await deleteServiceInquiry(inquiryToDelete);
  ...
  const markAsReplied = async () => {
    if (selectedInquiry) {
      try {
        const updatedInquiry = await updateServiceInquiryStatus(selectedInquiry.id, "replied");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Service Inquiries Management</h1>
      </div>
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{selectedInquiry?.serviceName}</DialogTitle>
          </DialogHeader>
          ...
          <Card className="shadow-none">
            <CardContent className="p-4 space-y-4">
              ...
              <div className="border-t pt-4">
                <p className="text-sm font-medium mb-2">Service Requirements:</p>
                <p className="whitespace-pre-wrap">{selectedInquiry?.serviceDetails}</p>
              </div>
